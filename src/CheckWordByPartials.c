...
unsigned char checkWord(const char *token_str, const char *base_str, char *partial_buff) 
{	
	char cur_char, next_char;
	const char *base_pointer=base_str;
	unsigned int token_index=0, partial_index;	
	do{	
		partial_index=0;
		do{	
			cur_char= token_str[token_index];
			next_char= token_str[token_index+1];		
		...
		memset(&partial_buff[0],0,partial_index * sizeof(char));  //resets partial_buff so it can collect the instances of the next char
